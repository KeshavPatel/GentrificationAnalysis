%% Steady State PDE Toolbox Implementation for PDE System
%{
Author:         Keshav Patel
...
function [ result ,model,dimensions, ICVec] = continuumSteadyState(R,E,B,S,A,initialConditionCell)
...
%model.SolverOptions.MaxIterations = 1;
...
switch n
    ...
    case 4
        u0(1,:) = c*double(abs(locations.x)<.01 & abs(locations.y)<.01);
        u0(2,:) = c*double(abs(locations.x)<.01 & abs(locations.y)<.01);
